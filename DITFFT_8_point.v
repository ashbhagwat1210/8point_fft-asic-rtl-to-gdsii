// ============================================================
// 8-point Radix-2 DIT FFT, fixed-point Q1.15, 3-stage pipeline
//
// BUS CONVENTION:
//   Input  (real samples only): 128 bits = 8 x 16b, sample k at
//           in_flat[127-16*k -: 16]
//   Internal / output (complex): 256 bits = 8 x (16b re + 16b im),
//           sample k at data[255-32*k -: 32], re in top 16, im in bottom 16
//   256 bits is required from stage 1 onward because every butterfly
//   output is complex, even though the input is real.
// ============================================================
...
module complex_mult (
    ...
    // (a_re + j*a_im)(w_re + j*w_im) = (rr-ii) + j(ri+ir)
...
module butterfly_w0 (
    ...
    // sign-extend to 17b before add/sub -> no overflow, then >>>1
    // for the standard per-stage FFT scaling (prevents bit growth).
...
// ---------------- Butterfly, W = -j ----------------
// (-j)*(re + j*im) = im - j*re  -> rotate then add/sub, no multiplier
...
// ---------------- Real-to-complex expansion + bit-reversal ----------------
// index map: 0->0, 1->4, 2->2, 3->6, 4->1, 5->5, 6->3, 7->7
module bit_reverse (
    input  [127:0] in_flat,          // 8 x 16b real samples
    output [255:0] data_out          // 8 x 32b complex, bit-reversed order
);
...
// ---------------- Stage 1: four W0 butterflies ----------------
// pairs (bit-reversed order): (0,1)(2,3)(4,5)(6,7), all W=1
module fft_stage1 (
    input  [255:0] data_in,
...
// ---------------- Stage 2: W0 and W(-j) butterflies ----------------
// two independent groups of 4 (0-3, 4-7); within each: (idx0,idx2)=W0, (idx1,idx3)=W(-j)
module fft_stage2 (
    input  [255:0] data_in,
    output [255:0] data_out
);
...
// ---------------- Stage 3: combine across the two groups ----------------
// (0,4)=W0, (1,5)=W1=(1-j)/sqrt2, (2,6)=W(-j), (3,7)=W3=(-1-j)/sqrt2
module fft_stage3 (
    input  [255:0] data_in,
    output [255:0] data_out
);
